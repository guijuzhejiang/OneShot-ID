...
**English README (默认)**: [README.md](README.md)
...
- **InsightFace**：`models.insightface_dir`（需 `antelopev2` 等）  
  - 下载（AntelopeV2）：[antelopev2.zip](https://github.com/deepinsight/insightface/releases/download/v0.7/antelopev2.zip)
- **InstantID**：`models.instantid_dir`（ControlNet + `ip-adapter.bin` 等）  
  - 下载（官方仓库）：[InstantX/InstantID](https://huggingface.co/InstantX/InstantID)
- **SDXL**：`models.sdxl_path`（本项目示例为 juggernautXL_ragnarokBy）  
  - 下载（示例模型页）：[Juggernaut XL (Civitai)](https://civitai.com/models/133005/juggernaut-xl)
